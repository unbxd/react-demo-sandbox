...
export const ProductItemComponent = (props) => {
  ...
  const [currentImageUrl, setCurrentUrl] = useState(
    firstSwatch ? formattedSwatches[firstSwatch]["swatchImageUrl"] : imageUrl
  );
  ...
  if (Object.keys(formattedSwatches).length > 1) {
    swatchMarkup = Object.keys(formattedSwatches).map((swatchColor) => {
      ...
      return (
        <div className="UNX-swatch__item" title={color} onClick={bindedOnClick}>
...
const SwatchProductItemComponent = (props) => {
  const { itemData, priceUnit, onClick } = props;
  const { imageUrl, title, price, sellingPrice, productUrl, swatches } =
    itemData;
  ...
  const [sizeSwatch, setSizeSwatch] = useState(Object.values(sizeSwatchesMap));
  ...
  const handleSizeSwatchClick = (currentSwatch) => {
    ...
    setSizeSwatch([...filteredSizeSwatches, { ...currentSizeSwatch, list }]);
  };
    ...
    <div className="UNX-productCard__container">
      <div className="details" onClick={handleClick}>
        ...
        <div className="-swatch__container -multiple">
          <div
            className={`-color__container ${isColorSwatchOpen ? "-open" : ""}`}
          >
          ...
          <div
            className={`-size__container ${isSizeSwatchOpen ? "-open" : ""}`}
          >
